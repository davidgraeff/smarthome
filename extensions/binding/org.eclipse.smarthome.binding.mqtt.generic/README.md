...
* __rgb__: Set this to true to use "Red,Green,Blue" (RGB) format, otherwise "Hue,Saturation,Brightness" (HSB) is used.
...
All mentioned channels can have a configured optional transformation.

This is required if your MQTT topic value is for example in the JSON format and you need to extract your desired value first.

For example we assume a received input of `{device: {status: { temperature: 23.2 }}}` and want to extract the temperature value.
This is a perfect match for the [JSONPath](http://goessner.net/articles/JsonPath/index.html#e2) transformation.
Head over to for example http://jsonpath.com/ to create your json path.

For the given JSON, you would use "JSONPATH:$.device.status.temperature".
